...
const shuffleCards = (level: number): Card[] => {
  ...
  const doubledCards = [...cardSubset, ...cardSubset]; // Duplique les cartes
...
const MemoryGame = () => {
  ...
  const renderCard = (card: Card) => {
    ...
    return (
      <div
        key={card.id}
        className={`w-24 h-24 flex items-center justify-center bg-gray-200 rounded-lg m-2 cursor-pointer transform ${
          isFlipped ? "bg-green-400" : "bg-gray-300"
        }`}
